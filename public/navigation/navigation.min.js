!(function () {
  ...
  function getElementsByAttributeName(elements, attribute) {
    for (var result = [], i = 0, n = elements.length; n > i; i++)
      if (
        (null !== elements[i].getAttribute(attribute) &&
          result.push(elements[i]),
        elements[i].children.length > 0)
      ) {
        var childrens = getElementsByAttributeName(
          elements[i].children,
          attribute
        );
        childrens.length > 0 && (result = result.concat(childrens));
      }
    return result;
  }
  ...
  function removeClass(element, className) {
    for (
      var classes = element.className.split(" "), i = classes.length - 1;
      i >= 0;
      i--
    )
      classes[i] == className && classes.splice(i, 1);
    ...
    if ("function" == typeof CustomEvent)
      ...
      if ("function" != typeof document.createEvent)
        throw (
          ...
          new Error("Can	 create custom event"))
        );
      (navEvent = document.createEvent("CustomEvent")),
        navEvent.initCustomEvent(eventName, !0, !0, { value: value });
    }
    target.dispatchEvent(navEvent);
  }
  if ("undefined" == typeof DEBUG) var DEBUG = !1;
  var prefix = "nv",
    options = {
      FOV: 90,
      prefix: prefix,
    ...
    Nav = function () {
      return Nav.instance
        ? Nav.instance
        : ((Nav.instance = this), Nav.instance);
    };
  (Nav.prototype.debug = function (flag) {
    return (DEBUG = flag);
  }),
    (Nav.prototype.isMouseEnable = !0),
    ...
    (Nav.prototype.initialize = function () {
      var self = this,
        body = document.body,
        navElements = getElementsByAttributeName(
          body.children,
          options.attrElement
        );
      return (
    ...
    (Nav.prototype._setCurrentScope = function (scope) {
      ...
      if (prevScope.name === scope.name)
        return (
          DEBUG && console.info(scope.name, ": scope is current now"), this
        );
      if (((this._prevScope = prevScope), this._prevScope)) {
        ...
        prevScopeEl.removeAttribute(options.attrScopeCurrent),
          ...
          prevElement && removeClass(prevElement, options.attrElementCurrent);
      }
      return (
    ...
    (Nav.prototype.getEventName = function (event) {
      for (
        var keyMapping = this.getKeyMapping(), i = 0;
        i < keyMapping.length;
        i++
      ) {
        var item = keyMapping[i];
        if (item[0] == event.keyCode) return item[1];
      }
      return !1;
    }),
    (Nav.prototype.getEventValue = function (event) {
      for (
        ...
        if (item[0] == event.keyCode)
          return "undefined" == typeof item[2] ? item[1] : item[2];
      }
      return !1;
    }),
    ...
    (Nav.prototype._addElementToScope = function (scopeName, element) {
      var self = this,
        scope = this.getScope(scopeName);
      if (!scope) throw Error('Scope "' + scopeName + '" not found');
      DEBUG && console.info(scope.name, ": add new element"),
        scope.addElement(element),
        element.setAttribute(
          "tabindex",
          (scope.getNavigationElements().length + 1).toString()
        );
      var _remove = element.remove;
      return (
        (element.remove = function () {
        ...
        scope.isCurrentElement(element) &&
          (DEBUG && console.info(scope.name, ": new element is current"),
          scope.setCurrentElement(element)),
        element.addEventListener("mouseover", function (event) {
          return self.isMouseEnable
            ? (scope.isCurrentElement(element) ||
                scope.setCurrentElement(element),
              self.isCurrentScope(scope) || self.changeScope(scope.name),
              void scope.setCurrentElement(element))
            : !1;
        }),
        self
      );
    ...
    (Nav.prototype.addElement = function (element) {
      for (
        var scopeName = null, _el = element, attrName = options.attrScope;
        null === scopeName && null !== _el.parentElement;

      )
        (scopeName = _el.parentElement.getAttribute(attrName)),
          (_el = _el.parentElement);
      if (!scopeName) throw new Error(element + " not belong to scope");
      var scope = this.getScope(scopeName);
      scope || this.addScope(_el),
    ...
    (Nav.prototype.hasElement = function (element) {
      ...
      for (var scopeName in scopes) {
        ...
        if (scope.hasElement(element)) return !0;
      }
      return !1;
    }),
    (Nav.prototype.getScopeByElement = function (element) {
      ...
      for (var scopeName in arguments) {
        var scope = scopes[scopeName];
        if (scope.hasElement(element)) return scope;
      }
      return !1;
    }),
    ...
    (Nav.prototype.onKeyDown = function (event) {
      ...
      if (!currentScope)
        return void (DEBUG && console.error("current scope not found"));
      var currentElement = currentScope.getCurrentElement();
      if (!currentElement)
        return void (DEBUG && console.info("current element not found"));
      var eventName = this.getEventName(event),
        eventValue = this.getEventValue(event);
      DEBUG &&
        console.log(
          "keycode: ",
          +event.keyCode,
        ...
        eventName &&
          ["left", "right", "up", "down"].indexOf(eventName) > -1 &&
          self.move(eventName),
        eventName && this.trigger(eventName, currentElement, eventValue);
      var log = document.getElementById("nav-event-info");
      log &&
        (log.innerHTML =
          "<b>Last event:</b><br /> navEvent: " +
          options.prefix +
          "-" +
          eventName +
          "<br /> keyValue: " +
    ...
    (Nav.prototype.move = function (direction) {
      var self = this,
        currentScope = self.getCurrentScope(),
        currentElement = currentScope.getCurrentElement(),
        nextElement = currentScope.getNextElement(direction);
      nextElement &&
        (currentScope.setCurrentElement(nextElement),
        this.trigger("move", currentElement),
        this.trigger("focus", nextElement)),
        self.mouseEnableTimeout && clearTimeout(self.mouseEnableTimeout),
        (self.isMouseEnable = !1),
        (self.mouseEnableTimeout = setTimeout(function () {
          self.isMouseEnable = !0;
        }, 1e3));
    }),
    ...
    (Nav.prototype.scrollToCurrentElement = function () {
      var scope = this.getCurrentScope(),
        element = scope.getCurrentElement();
  ...
  (NavScope.prototype.activate = function () {
    ...
    (NavScope.prototype.getCurrentElement = function () {
      if (this.currentElement) return this.currentElement;
      for (
        var navElements = this.navigationElements,
          currentElement = !1,
          i = 0,
          n = navElements.length;
        n > i;
        i++
      )
        this.isCurrentElement(navElements[i]) &&
    ...
    (NavScope.prototype.getNextElement = function (direction) {
      ...
      if (!current)
        return DEBUG && console.info("Current element not found"), !1;
      if (-1 === ["up", "down", "left", "right"].indexOf(direction)) {
        var error = "Not declared direction: " + direction;
        throw (DEBUG && console.error(error), Error(error));
      }
      var index,
        windowSize = getWindowSize(),
        currentElementFOV = parseInt(
          current.getAttribute(options.attrElementFOV),
          10
        ),
      ...
      "up" == direction
        ...
        : "down" == direction
        ? (currentElementCenter.y =
            currentElementRect.top + currentElementRect.height)
        : "left" == direction
        ? (currentElementCenter.x = currentElementRect.left)
        : "right" == direction &&
          (currentElementCenter.x =
            currentElementRect.left + currentElementRect.width);
      for (var i = 0; i < navElements.length; i++) {
        var el = navElements[i],
          isHidden = null == el.offsetParent,
        ...
        if (!isHidden && !isCurrent && inDOM) {
          ...
          var inFOV = !1;
          "left" == direction &&
          ((angle > 360 - halfFOV && 360 >= angle) ||
            (angle >= 0 && 0 + halfFOV > angle))
            ? (inFOV = !0)
            : "down" == direction &&
              angle > 270 - halfFOV &&
              270 + halfFOV > angle
          ...
          intersection &&
            ...
            (inFOV || intersections.length) &&
              (null === distance || distance > elementDistance) &&
              ((distance = elementDistance), (index = i));
        }
      }
      if ("undefined" == typeof index) return !1;
      var nextElement = navElements[index];
      return scroll(this.element, nextElement), nextElement;
    });
  var nav = new Nav();
